...
public class HeterogeneousMaxHolder {
     ...
     */
    public <T extends Comparable<? super T>> T put(@NonNull  Class<T> key, @NonNull T value) {
     ...
     * @return a smaller value among the provided value and the current maximum
     */
    public <T> T put(@NonNull Class<T> key, @NonNull T value, @NonNull Comparator<? super T> comparator) {
        T currentValue = getMax(key);
        if (currentValue == null || Comparator.nullsLast(comparator).compare(currentValue, (T) value) < 0) {
            map.put(key, value);
            return currentValue;
        }
        return value;
     ...
     */
    public <T> T getMax(@NonNull Class<T> key) {
